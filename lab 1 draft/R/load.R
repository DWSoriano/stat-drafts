...
loadLingData <- function(path = "data/") {
  # Arguments:
  #   path: the path indicating the location of the `lingData` 
  #         data file.
  #         Path should be relative to the lab2.Rmd file.
  # Returns:
  #   a data frame consisting of lingData dataset which contains the answers to 
  #   the questions for 47,471 respondents across the United States from the 
  #   Dialect Survey
  ling_data <- read.table(paste0(path, "lingData.txt"), header = TRUE, 
                     stringsAsFactors = FALSE, colClasses=c("ZIP"="character"))
...
loadLingLocation <- function(path = "data/") {
  # Arguments:
  #   path: the path indicating the location of the `lingLocation` 
  #         data file.
  #         Path should be relative to the lab2.Rmd file.
  # Returns:
  #   a data frame consisting of location dataset 
...
loadDatesData <- function(path = "data/") {
  ...
  epoch_df <- data.frame(number = as.numeric(as.character(epoch_nums)),
                         date = epoch_dates,
                         day = epoch_days)
...
loadMoteLocationData <- function(path = "data/") {
  # Arguments:
  #   path: the path indicating the location of the `mote-location-data` 
  #         data file.
  #         Path should be relative to the lab1.Rnw file.
  # Returns:
